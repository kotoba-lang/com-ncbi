(ns ncbi.main
  "Kotodama WASM entrypoint for the NCBI clean-room actor (L4) — Clojure port.
  ...
  facts (`ncbi.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ncbi.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ncbi-compat" :tier tier :entities entities} 200])
